...
RUN apt-get update \
    ...
    && apt-get clean
CMD python /matrix_archive/matrix-archive.py /matrix_archive/chats --batch --server $SERVER --user "$USER" --userpass "$USERPASS" --keys /matrix_archive/key.txt --keyspass "$KEYPASS" $ROOMSTR